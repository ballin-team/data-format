...
import { validCamelDummies, validCamelDummy } from './dummies/camelToSnake.dummy';
...
describe('Input validations', () => {
  ...
  it('Should call modifyObjectKeysWithCache if useCache is true', () => {
    ...
    camelToSnake(validCamelDummy, true);
  ...
  it('Should call modifyObjectKeys if useCache is false', () => {
    const modifyObjectKeysSpy = jest.spyOn(helper_functions, 'modifyObjectKeys');
    const modifyObjectKeysWithCacheSpy = jest.spyOn(helper_functions, 'modifyObjectKeysWithCache');
    camelToSnake(validCamelDummy, false);
...
describe('Execution validation', () => {
  it.each(validCamelDummies)('Should a success response %p', (_, input, response) => {
    expect(camelToSnake(input)).toEqual(response);
  });

  it.each(validCamelDummies)('Should a success response %p (cache version)', (_, input, response) => {
    expect(camelToSnake(input, true)).toEqual(response);
  });
});
